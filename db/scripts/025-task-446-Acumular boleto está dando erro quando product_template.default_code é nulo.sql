...
/*
 Versão:2024.03.23
   task-446-Acumular boleto está dando erro quando product_template.default_code é nulo
 Versão:2024.02.21
                   ...
                   from (select invoice_destino.id invoice_destino_id,
                           ...
                           from (select anomes_inc(_anomes_destino, -1) anomes_1, /*mes passado (com relacao ao _anomes_destino) */
                                ...
                                (select invoice_destino1.id,
                                  ...
                                  where invoice_destino1.state = 'draft'
                                 ...
                                 account_invoice_line invoice_line_origem
                                   join product_product pdt on pdt.id = invoice_line_origem.product_id and coalesce(pdt.default_code, '') not in ('PROPCM', 'PROPJU', 'PROPMJ', 'PROPMU', 'PROPTP')
