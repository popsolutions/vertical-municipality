...
AS SELECT pl.id,
    ...
    COALESCE(pl.invoicesend_email,
        CASE
            WHEN owner_invoice.id IS NULL THEN COALESCE(res_owner.invoicesend_email, res_owner.email)
            ELSE COALESCE(owner_invoice.invoicesend_email, owner_invoice.email)
        END) AS email_boleto_calc
   FROM property_land pl
     ...
     LEFT JOIN res_partner owner_invoice ON owner_invoice.id = pl.owner_invoice_id
  WHERE pl.active;