...
class AccountInvoice(models.Model):
    ...
    def process_property_water_catchment(self):
        ...
        property_wc_ids = self.env['property.water.catchment'].search(
        ...
        for p_wc in self.web_progress_iter(property_wc_ids, msg="Process Water Catchment"):
            if p_wc.land_id.id not in inv_land_ids:
                ...
                inv_id.write({'invoice_line_ids': [(0, 0, {
                    ...
                    'price_unit': p_wc.rate_catchment,
            ...
            records_index += 1
            print('account.invoice - property.water.catchment - record ' + str(records_index) + '/' + str(records_len))


    @api.multi
    def _create_property_wcc_customer_invoice(self, p_wc, product_id, account_id):
